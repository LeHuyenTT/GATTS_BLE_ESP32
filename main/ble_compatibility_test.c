...
#define SAMPLE_DEVICE_NAME          "LOP 2 - NHOM 2"
...
#define GATTS_NOTIFY_FIRST_PACKET_LEN_MAX 50
...
void append_strings(char** a, const char* b) {
    // Tính độ dài mới cho a bằng cách tính tổng độ dài của a và b
    char* new_a = (char*)malloc(strlen(*a) + strlen(b) + 1);
    
    // Sao chép nội dung của a vào new_a
    strcpy(new_a, *a);
    
    // Thêm nội dung của b vào new_a
    strcat(new_a, b);

    // Giải phóng vùng nhớ cũ của a
    free(*a);

    // Cập nhật con trỏ a để trỏ đến new_a
    *a = new_a;
}
...
/* Service */
static esp_gatt_status_t gatts_read_cb(uint16_t conn_id, esp_ble_gatts_cb_param_t *param);

#define IDX_CHAR_VAL_B_HANDLE 0x5678
...
static const uint8_t char_ccc[2]   = {0x02, 0x00};
static const uint8_t char_value[] =  "Nhap ID";
...
static const esp_gatts_attr_db_t gatt_db[HRS_IDX_NB] =
{
    ...
    [IDX_CHAR_A]     =
    {{ESP_GATT_APP_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_declaration_uuid, ESP_GATT_PERM_READ,
...
esp_gatt_if_t gatts_if_1;
uint16_t descr_value1 = 0x0000;
...
void task_ssd1306_display_text(const char *arg_text) {
...
static esp_gatt_status_t gatts_read_cb(uint16_t conn_id, esp_ble_gatts_cb_param_t *param)
{
    ...
    uint8_t response_data[16] = ""; // Khai báo biến response_data

    if (param->read.handle == gatt_db_handle_table[IDX_CHAR_VAL_B]) {
        // ESP_LOGI("GATT", "Read event received");
        // ESP_LOGI("GATT", "Read handle: %d", param->read.handle);

        // Xử lý yêu cầu đọc thuộc tính B
        if (descr_value1 == 0x0000) {
            memcpy(response_data, "HELLO", sizeof("HELLO"));
        } else if (descr_value1 == 0x0001) {
            memcpy(response_data, "Please Input ID", sizeof("Please Input ID"));
        }

        esp_gatt_rsp_t rsp;
        memset(&rsp, 0, sizeof(esp_gatt_rsp_t));
        rsp.attr_value.handle = param->read.handle;
        rsp.attr_value.len = sizeof(response_data);
        memcpy(rsp.attr_value.value, response_data, sizeof(response_data));
        esp_ble_gatts_send_response(gatts_if_1, param->read.conn_id, param->read.trans_id, ESP_GATT_OK, &rsp);
    } else {
    ...
    switch (event) {
        ...
        case ESP_GATTS_READ_EVT:
         //ESP_LOGE(EXAMPLE_TAG, "ESP_GATTS_READ_EVT, handle=0x%d, offset=%d", param->read.handle, param->read.offset);
                ...
                gatts_read_cb(param->read.conn_id, param);
        ...
        case ESP_GATTS_WRITE_EVT:
            if (!param->write.is_prep){
                ...
                if (gatt_db_handle_table[IDX_CHAR_VAL_B] == param->write.handle ){
                    ...
                    descr_value1=descr_value;
                    //ESP_LOGE(EXAMPLE_TAG, "(5) ***** write char2 ***** \n");
                    if (descr_value == 0x0001){
                        //the size of notify_data[] need less than MTU size
                        
                        esp_ble_gatts_send_indicate(gatts_if, param->write.conn_id, gatt_db_handle_table[IDX_CHAR_VAL_B],
                                                sizeof(notify_data), notify_data, false);
                        ESP_LOGI(EXAMPLE_TAG, "(6) ***** Client want to enter their ID : ***** \n");
                    }
                        ...
                        uint32_t ID = param->write.value[0]<<24 | param->write.value[1]<<16 | param->write.value[2]<<8 | param->write.value[3];
                        char ID_string[9];
                        sprintf(ID_string, "%08X", ID);
                        ESP_LOGI(EXAMPLE_TAG, "Client have ID: %04X", ID);
                        strcat(a,ID_string);
                        strcat(a, "\n");
                        task_ssd1306_display_text(a);
                        printf("ID: %s\n", ID_string);
                    }
                }
...
void app_main(void)
{
   ...
   // ESP_ERROR_CHECK(nvs_flash_erase());
    ...
    //char utf8_string[32] = "12345";
   // task_ssd1306_display_text(utf8_string);
